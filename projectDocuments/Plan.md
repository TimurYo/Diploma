...
### 3.5 Проверка идентичности значения в столбце transaction id в таблице payment_entity и payment id в таблице order_entity
...
### 3.6 Проверка идентичности значения в столбце bank id в таблице credit_payment_entity и credit id в таблице order_entity 
...
### 3.7 Проверка идентичности значения времени и даты записи в столбце created в таблице payment_entity и created в таблице order_entity
...
### 3.8 Проверка идентичности значения времени и даты записи в столбце created в таблице credit_payment_entity и created в таблице order_entity
...
IDEA - знакомая среда для разработки. Она использовалась нами на протяжении всего обучения, на ее основе нам показывали
работу различных инструментов для создания авто-тестов. Она обладает автоматическим подключением к Git, что упрощает 
работу. Также для этого проекта потребуется выполнение запросов в базу данных, а IDEA предоставляет возможность подключения
плагина, позволяющего сделать это.

* ### Git и GitHub

Связка Git и GitHub - необходимая часть разработки для дальнейшего распространения продукта и возможности редактирования 
его на всех этапах разработки.

* ### Docker Desktop

Так как SUT требует создание и подключения контейнеров с двумя СУБД и gate-simulator, docker необходим для включения 
тестируемой системы

* ### DBeaver

Под тестирование попадают системы управления базами данных, поэтому DBeaver необходим для отображения таблиц и хранящихся 
в них данных, оценки работы SUT нативно, без использования программного кода.

* ### Allure

Для создания отчета была выбрана система Allure, так как она более информативна и репрезентативна, чем Gradle report, и 
менее сложная для подключения Report Portal.

* ### Appveyor

В перспективе проект может быть основой для командной разработки, поэтому необходима среда для CI. 
В качестве такой среды был выбран Appveyor ввиду возможности показа бейджа с результатом сборки, а также просмотра
лог - журнала создания проекта в виртуальной среде.

* ### Язык программирования - Java

Выбрана Java в связи с тем, что данный язык зарекомендовал себя наиболее приемлемым для разработки авто-тестов в процессе
обучения на курсе.

* ### Система сборки - Gradle

Выбран система сборки Gradle вместо Maven в связи с тем, что данная система проще в настройке (создание build.gradle 
лаконичнее в отличие от Maven).

* ### Тестовый фреймворк - JUnit-5

JUnit обладает широким спектром необходимых аннотаций (методы создания параметров для тестов, методы подключения
к базам данных, методы тестирования временных значений, методы для тестирования веб-сервисов и т.п.) для тестирования.

## 3. Перечень и описание возможных рисков при автоматизации

1. Проблемы, связанные с поднятием SUT (разбор способа запуска основного приложения, создание контейнеров для mySQL, 
postgreSQL и gate-simulator)
2. Сложности с успешной реализацией автоматического тестирования баз данных
3. Сложности с реализацией универсального способа тестирования БД с MySQL и БД c PostgreSQL.
4. Сложности с тестированием времени записи данных в базе данных.
5. Сложности с определением необходимых CSS-селекторов для тестирования веб-клиента
6. Сложности реализации CI

## 4. Интервальная оценка с учетом рисков в часах

1. Изучение основного и дополнительного теоретического материала - от 15 до 20 часов
2. Написание авто-тестов - от 40 до 45 часов
3. Формирование баг - репортов - от 2 до 3 часов
4. Формирование документа "Отчет по итогам тестирования" - от 4 до 6 часов
5. Формирование документа "Комплексный отчет по итогам всего процесса тестирования" - от 5 до 8 часов

Итог: 
* минимальное количество часов, необходимое для реализации проекта - 66 часов
* максимальное количество часов, необходимое для реализации проекта - 82 часа

## 5. План сдачи работ

В связи с тем, что работа над проектом сопряжена с основной работой (врачебная деятельность), сроки сдачи работ будут 
исчисляться в рабочих дня, равных 8 часам работы, но по реальности эти сроки могут исчисляться неделями.
1. Реализация авто-тестов - в течение 9-12 рабочих дней после утверждения данного плана
2. Предоставление отчета о тестировании - в течение 1-2 рабочих дней после проведения автоматизированного тестирования SUT
3. Предоставление комплексного отчета по итогам тестирования - в течение 1-2 рабочих дней после подготовки отчета о
тестировании

Итог: 
* минимальное количество дней для предоставления готового проекта - 10 рабочих дней
* максимальное количество дней для предоставления готового проекта - 16 рабочих дней 
